...
public static function checkLoginCredentials($email, $password, $tableName, $conn) {
    mysqli_select_db($conn, "dartoda1");
    // Fetch user data based on email
    $sql = "SELECT id, firstname, lastname, email, password, idCity FROM $tableName WHERE email='$email'";
    $result = mysqli_query($conn, $sql);
...
if (mysqli_query($conn, $sql)) {
    ...
    public static function authenticateClient($tableName, $conn, $email, $password) {
